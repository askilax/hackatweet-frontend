        ...
        <div className={styles.tweetsList}>
            {sortedTweets.length > 0 ? (
                sortedTweets.map((tweet) => (
                    <Tweet key={tweet.id} tweet={tweet} firstName={firstName} userName={userName} />
                ))
            ) : (
...
// Composant pour un tweet individuel avec gestion des likes
const Tweet = ({ tweet, firstName, userName }) => {
